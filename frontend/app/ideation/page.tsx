...
export default function IdeationPage() {
  ...
  function selectAllExtracted() {
    setSelectedPaperIds(
      extractedPapers.map((paper) => paper.id),
    );
  }
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 text-sm leading-6 text-slate-600">
                Analyze limitations, future work, methodologies, and findings across your selected corpus.
                The output is a set of candidate gaps and testable research directions grounded in the papers you select.
              </p>
            </div>
            ...
            <button
              ...
              disabled={generating || selectedPaperIds.length === 0}
            ...
            >
              ...
              {generating ? "Analyzing corpus..." : "Generate ideas"}
            </button>
          </div>
        </section>
          ...
          <div className="flex flex-col gap-4 border-b border-slate-100 pb-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Select papers that have completed extraction. More diverse, relevant papers generally produce a stronger corpus comparison.
              </p>
            </div>
            ...
            <div className="space-y-4 rounded-2xl bg-slate-50 p-5">
              ...
              <div className="rounded-xl border border-violet-100 bg-violet-50 px-4 py-3 text-sm text-violet-900">
                ...
                <p className="mt-1 text-xs leading-5 text-violet-700">
                  Every generated idea should reference source paper IDs, so you can trace it back to the selected corpus.
                </p>
              </div>
            </div>
          </div>
          ...
          <>
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-5">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Analysis based on {result.corpus_size} selected paper{result.corpus_size === 1 ? "" : "s"} using {result.model}.
                </p>
              </div>
              ...
              <div className="grid gap-4 lg:grid-cols-3">
                <div className="rounded-2xl border border-orange-100 bg-orange-50 p-5">
                  ...
                  <div className="mt-3 space-y-2">
                    {result.recurring_limitations.length > 0 ? (
                      result.recurring_limitations.map((item, index) => (
                        <p key={`${item}-${index}`} className="text-sm leading-6 text-orange-900">
                          {index + 1}. {item}
                        </p>
                      ))
                    ) : (
                      <p className="text-sm text-orange-800">No recurring limitations were identified from the available extraction fields.</p>
                    )}
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-violet-100 bg-violet-50 p-5">
                  ...
                  <div className="mt-3 space-y-2">
                    {result.research_gaps.length > 0 ? (
                      result.research_gaps.map((item, index) => (
                        <p key={`${item}-${index}`} className="text-sm leading-6 text-violet-900">
                          {index + 1}. {item}
                        </p>
                      ))
                    ) : (
                      <p className="text-sm text-violet-800">No candidate gaps were returned. Ensure selected papers have limitations and future-work extraction data.</p>
                    )}
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-cyan-100 bg-cyan-50 p-5">
                  ...
                  <div className="mt-3 space-y-2">
                    {result.recommended_methodologies.length > 0 ? (
                      result.recommended_methodologies.map((item, index) => (
                        <p key={`${item}-${index}`} className="text-sm leading-6 text-cyan-900">
                          {index + 1}. {item}
                        </p>
                      ))
                    ) : (
                      <p className="text-sm text-cyan-800">No method directions were returned from the corpus.</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-5">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  These are research directions to validate through a fuller literature review, not claims of guaranteed novelty.
                </p>
              </div>
                    ...
                    >
                      ...
                      <div className="mt-5 border-t border-slate-200 pt-4">
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          {idea.evidence_paper_ids.length > 0 ? (
                            idea.evidence_paper_ids.map((paperId) => (
                              <span
                                key={paperId}
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-left text-sm">
                  ...
                  <tbody>
                    {result.evidence_papers.map((paper) => (
                      <tr key={paper.paper_id} className="border-t border-slate-100 align-top">
                        <td className="px-4 py-4">
                          <p className="font-medium text-slate-900">{paper.title}</p>
                          <p className="mt-1 break-all text-xs text-slate-500">{paper.paper_id}</p>
                        </td>
                        <td className="max-w-xs px-4 py-4 leading-6 text-slate-600">{paper.limitation || "—"}</td>
                        <td className="max-w-xs px-4 py-4 leading-6 text-slate-600">{paper.future_work || "—"}</td>
                        <td className="max-w-xs px-4 py-4 leading-6 text-slate-600">{paper.methodology || "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
