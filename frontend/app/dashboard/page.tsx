...
import { ArrowRight, FileText, Database, Sparkles, SearchCheck } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const stats = [
    { label: "Total papers", value: total, icon: FileText, color: "bg-blue-50 text-blue-700" },
    { label: "Parsed", value: parsed, icon: Database, color: "bg-cyan-50 text-cyan-700" },
    { label: "Indexed", value: indexed, icon: SearchCheck, color: "bg-emerald-50 text-emerald-700" },
    { label: "Extracted", value: extracted, icon: Sparkles, color: "bg-violet-50 text-violet-700" },
  ];
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
            <div className="max-w-2xl">
              ...
              <h2 className="text-4xl font-semibold tracking-tight text-slate-900">
                Organize papers into structured, searchable research knowledge
              </h2>
              <p className="mt-4 text-base leading-7 text-slate-600">
                Upload papers, parse sections, retrieve relevant evidence, and compare methods,
                datasets, metrics, and limitations in one workspace.
              </p>
            </div>
          </div>
                ...
                {loading ? (
                  ...
                  papers.map((paper) => (
                    <tr key={paper.id} className="border-t border-slate-100">
                      ...
                      <td className="px-6 py-4 text-slate-600">{paper.year || "—"}</td>
                      <td className="px-6 py-4">
                        <Link
